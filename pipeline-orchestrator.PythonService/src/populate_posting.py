...
df['WeightsId'] = [str(uuid.uuid4()) for _ in
                   range(len(df))]  # first create this unique GUID mapping and then separate the table
df['ListingId'] = [str(uuid.uuid4()) for _ in range(len(df))]
df_cols = df.columns.tolist()

weight_cols = ['Weight_SkillMatch', 'Weight_SkillDepth', 'Weight_Experience', 'Weight_Education',
               'Weight_Certifications', 'Weight_ActiveContribs']
listing_table = ['ListingCountry', 'ListingCity', 'ListingAddress', 'ListingLat', 'ListingLong', 'IsRemote',
                 'RelocateAvailable']
...
# Let's assign navigational property (foreign key) to these tables
...
df_posting = df_posting.reindex(columns=lower_array(
    posting_sql_columns))  # make sure the order of the column is same as sql table, otherwise it will mess up the data and we don't want that.
...
def export_to_csv():
    if not os.path.isfile(posting_export_path) and not os.path.isfile(weights_export_path) and not os.path.isfile(
            listing_export_path):  # if already then do not override this
